function dataTrans = DataPDS_Fixed(dataTrain,targetTrain, dataTest, targetTest,segments)
    ...
    UGMsTrans = unique(targetTest.train);
    targetUGMs = UGMsTrans; % Selected Transfer Samples
    ...
    multiplyerUGM = sum(ismember(targetTrain.train,UGMsTrans))/sum(ismember(targetTest.train,UGMsTrans)); 
    if floor(multiplyerUGM) ~= multiplyerUGM
        disp("Warning, multiple UGMs have same target might cause problem")
    end
    ...
    for i = 1:size(targetUGMs,1)
        for j = 1:size(masterSensorArray,2)
            ...
            %Check of inconsistancie
            temp1 = dataTrainTrainCells{1,j}(targetTrain.train==targetUGMs(i),:);
            temp2 = dataTestTrainCells{1,j}(targetTest.train==targetUGMs(i),:);

            masterSensorArray{1,j} = [masterSensorArray{1,j};temp1(1:size(temp2,1),:)];
